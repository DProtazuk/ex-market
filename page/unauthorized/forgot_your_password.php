    ...
    <div class="my-content">
        ...
        <div class="col-12 content-body">
            <div class="myContainer mx-auto my-2">
                ...
                <form action="/backend/entry_system/forgot_your_password.php" method="post">
                    <div class="col-5 rounded-3 bg-silver mx-auto my-5 p-5">
                        ...
                        <?php
                            if(isset($_GET['status'])){
                                echo "<h6 class='text-center col-8 mx-auto'>Сообщение успешно отправлено вам на почту!</h6>";
                            }
                            if(isset($_GET['error'])){
                                if($_GET['error'] === "time"){
                                    $unix_time = $_COOKIE['time'] - time();
                                    if($unix_time < 300) {
                                        $formatted_time = date("i:s", $unix_time);
                                        echo "<h6 class='text-center col-8 mx-auto text-time'>Попробуйте через <span id='timer' class='text_blue'> ".$formatted_time."</span></h6>";
                                        echo '<script>$(document).ready(function() {
        var seconds = parseFloat('.$unix_time.');
        var interval = setInterval(function() {
            seconds--;
            var minutes = Math.floor(seconds / 60);
            var remainingSeconds = seconds % 60;
            var time = (minutes < 10 ? "0" + minutes : minutes) + ":" + (remainingSeconds < 10 ? "0" + remainingSeconds : remainingSeconds);
            $("#timer").text(time);
            if (seconds == 0) {
