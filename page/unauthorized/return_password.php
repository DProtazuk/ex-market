    ...
    <div class="my-content">
        ...
        <div class="col-12 content-body">
            <div class="myContainer mx-auto my-2">
                ...
                <form action="/backend/entry_system/return_password.php" method="post">
                    <div class="col-5 rounded-3 bg-silver mx-auto my-5 p-5">
                        ...
                        <?php
                            if(isset($_GET['key'])){
                                ...
                                $sth->execute(array($_GET['key']));
                                $array = $sth->fetch(PDO::FETCH_ASSOC);

                                if($array){
                                    $time = time() - $array['time'];
                                    if ($time > 300){
                                        echo ' <h6 class="text-center text-danger">Введенная ссылка уже не актуальна!</h6>

                        <a href="/page/unauthorized/forgot_your_password.php" type="submit"
                                class="bg_blue rounded-3 my-4 text-decoration-none d-flex justify-content-center text-center align-items-center col-12 text-16 py-1 text-white border_blue bg-transparent">
                            Попробовать еще раз
                        </a>';
                                    }
                                    else {
                                        ...
                                        if(isset($_GET['error']) AND $_GET['error'] === "password"){
                                            $error = '<h6 class="text-13 my-1 text-danger text-check-politics">Пароли не совпадают!</h6>';
                                        }
                                        else $error = NULL;

                                        echo '<div class="input-price-seller col-12 my-4">
                            <input value="'.$password.'" name="password" required placeholder="Введите Новый пароль" class="input-auth px-3 text-white bg-silver col-12 text-14 py-1 rounded-2 border border-secondary" type="text"
                                   style=" outline:none;  ">
                        </div>

                        <div class="input-price-seller col-12 my-4">
                            <input name="second_password" required placeholder="Повторите пароль" class="input-auth px-3 text-white bg-silver col-12 text-14 py-1 rounded-2 border border-secondary" type="text"
                                   style=" outline:none;  ">
                            '.$error.'

                        </div>
                        
                        <input type="hidden" name="link" value="'.$_GET['key'].'">
                        ...
                        <h6 class="text-14 my-3 text-center">Есть аккаунт? <a class="text-decoration-none text_blue"
                                                                              href="/page/unauthorized/authorization">Войти</a>
                        </h6>
